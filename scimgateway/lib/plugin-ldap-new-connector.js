...
const doRequest = async (baseEntity, method, base, ldapOptions, ctx) => {
  ...
  if (
    ...
    options.modification.userPrincipalName &&
    config.map.user.userPrincipalName &&
    config.map.user.userPrincipalName.mapDomain
  ) {
    if (
      options.modification.userPrincipalName.endsWith(
        config.map.user.userPrincipalName.mapDomain.outbound
      )
    ) {
      const old = options.modification.userPrincipalName;
      options.modification.userPrincipalName =
        options.modification.userPrincipalName.replace(
          config.map.user.userPrincipalName.mapDomain.outbound,
          config.map.user.userPrincipalName.mapDomain.inbound
        );
      scimgateway.logger.debug(
        `${pluginName}[${baseEntity}] inbound upnMapDomain ${old} => ${options.modification.userPrincipalName}`
      );
    }
  }
          ...
          client.search(base, options, (err, search) => {
            ...
            search.on("searchEntry", (entry) => {
              ...
              if (
                ...
                if (
                  obj.userPrincipalName.endsWith(
                    config.map.user.userPrincipalName.mapDomain.inbound
                  )
                ) {
                  const old = obj.userPrincipalName;
                  obj.userPrincipalName = obj.userPrincipalName.replace(
                    config.map.user.userPrincipalName.mapDomain.inbound,
                    config.map.user.userPrincipalName.mapDomain.outbound
                  );
                  scimgateway.logger.debug(
                    `${pluginName}[${baseEntity}] outbound upnMapDomain ${old} => ${obj.userPrincipalName}`
                  );
                }
              }
              results.push(obj);
        ...
        result = await new Promise((resolve, reject) => {
          ...
          for (const key in options.modification) {
            ...
            if (Array.isArray(options.modification[key]))
              ...
              if (typeof options.modification[key] === "string")
                mod.values = [options.modification[key]];
